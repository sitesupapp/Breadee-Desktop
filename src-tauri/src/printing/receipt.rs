...
pub struct ReceiptDoc {
    ...
    /// paper can show it on its own line. `None` (or 0) prints nothing. Display
    /// only — the total is authoritative and is never recomputed here.
    #[serde(default)]
...
pub fn build_receipt_page(doc: &ReceiptDoc, paper: PaperWidth) -> Vec<PageLine> {
    ...
    // Display only: the fee is already inside `total`, which is never recomputed.
...
mod tests {
    ...
    fn a_delivery_fee_prints_on_its_own_line_when_the_server_charged_one() {
        ...
        // the fee is display only, already inside the authoritative total.
