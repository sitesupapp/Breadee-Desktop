...
export function PaymentDialog(props: PaymentDialogProps) {
  ...
  // total - is what the receivable is measured against. It is the SAME canonical
  // payable the full-pay path uses (`payableInPrimary` = discount.finalTotal +
  // feeValue): the settlement mode may differ, but the customer's obligation must
  // not. For a delivery order this INCLUDES the delivery fee; for everything else
  // feeValue is 0 and this is byte-identical to the pre-fee behaviour.
  const dueInPrimary = payableInPrimary;
