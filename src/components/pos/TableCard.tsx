...
const STATE_STYLE: Record<TableCardState, { label: string; icon: string; ring: string; badge: string }> = {
  available: { label: "Free", icon: "○", ring: "border-line bg-white", badge: "bg-slate-100 text-slate-700" },
  occupied: { label: "Occupied", icon: "●", ring: "border-amber-300 bg-amber-50/60", badge: "bg-amber-100 text-amber-800" },
  active_bill: { label: "Open bill", icon: "▣", ring: "border-brand bg-brand-soft/40", badge: "bg-brand-soft text-brand-dark" },
  mixed_currency: { label: "Mixed currency", icon: "⚠", ring: "border-red-300 bg-red-50/70", badge: "bg-red-100 text-red-800" },
  reserved: { label: "Reserved", icon: "◐", ring: "border-sky-200 bg-sky-50/40", badge: "bg-sky-100 text-sky-800" },
  unknown: { label: "Unknown", icon: "?", ring: "border-line bg-white", badge: "bg-slate-100 text-slate-600" },
};
  ...
  return (
    ...
    >
      <span className="flex items-start justify-between gap-2">
        ...
        {table.seats != null && (
          <span className="shrink-0 text-[11px] font-semibold text-sub" title={`${table.seats} seats`}>
            {table.seats}p
          </span>
        )}
      </span>
