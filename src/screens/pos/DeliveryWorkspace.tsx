...
/**
 * Parse the manual delivery-fee input entered on the order before it is sent.
 *
 * The fee belongs to the ORDER, not only to the Pay action, so it is validated
 * exactly like a money field: blank means "no fee" (nothing is sent, the server
 * leaves the order feeless); a number is valid only when finite and >= 0. `0` is
 * a real value — free delivery — and is sent. Pure, so the send path can snapshot
 * and validate it without touching React state.
 */
export function parseDeliveryFee(raw: string): { provided: boolean; valid: boolean; value: number } {
  const trimmed = raw.trim();
  if (trimmed === "") return { provided: false, valid: true, value: 0 };
  const value = Number(trimmed);
  const valid = Number.isFinite(value) && value >= 0;
  return { provided: true, valid, value: valid ? value : 0 };
}
...
export function useDeliveryWorkspace(input: {
  ...
  // persists it and applies the finance layer. Empty = no fee.
      ...
      try {
        ...
        input.onPresentReceipt(
          buildReceipt({
            ...
            // The order's own persisted delivery fee, already folded into the
            // total by the finance engine — its own receipt line so the bill
            // reconciles. 0/absent prints nothing; never total - subtotal.
            deliveryFee: settled!.delivery_fee ?? null,
      ...
      try {
        ...
        input.onPresentReceipt(
          buildReceipt({
            ...
            subtotal: money?.subtotal ?? settled.total_amount ?? 0,
            discount: money?.discount ?? 0,
            // The order's own persisted delivery fee, shown as its own line so the
            // bill reconciles. 0/absent prints nothing; never total - subtotal.
            deliveryFee: settled.delivery_fee ?? null,
            total,
   ...
   * reuses the same value (no second, additive charge). Blank = no fee; 0 = free
   * delivery.
   */
