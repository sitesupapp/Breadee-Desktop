...
export function useDineInWorkspace(input: {
  ...
  const dialogs = (
    <>
      ...
      {/* Returning from the Designer re-reads the PUBLISHED Service Floor so a
          just-published layout is live immediately — no List↔Map toggle, branch
          switch, restart or 60s wait. Reuses the existing floor.load(ctx): the
          ServiceFloor stays mounted behind the overlay, so its own mount effect
          does not re-run on close. One harmless read on any close is acceptable;
          no polling, no second cache. */}
      <FloorDesigner
        open={editingFloor}
        ctx={ctx}
        onClose={() => {
