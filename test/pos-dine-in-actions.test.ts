...
test("settlement joined PosRpcName exactly once, and nothing else came with it", () => {
  ...
  // AND AGAIN BY DELIVERY SETTLEMENT WS7B: 23 -> 24, for `pos_delivery_resolve_cost`
  // (the post-close first-cost resolution; cash-inert, server-authoritative).
  assert.equal(members.length, 24, `the RPC allow-list changed size: ${members.join(", ")}`);
  assert.ok(members.includes("pos_delivery_resolve_cost"), "pos_delivery_resolve_cost is not callable - WS7B cannot resolve a cost");
