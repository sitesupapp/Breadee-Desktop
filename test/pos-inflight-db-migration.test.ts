...
test("Dexie v1 -> v2 upgrade preserves outbox / snapshots / audit and adds inflightSubmits", async () => {
  ...
  // 2) Open the app's database, which declares version(2). This triggers the real
  //    in-place upgrade of the existing v1 data.
  ...
  assert.equal(localdb.verno, 2, "database upgraded to version 2");
  ...
  assert.equal(inflight.length, 0, "new inflightSubmits store present and empty");
  localdb.close();
});
