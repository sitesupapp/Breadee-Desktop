// Restart-offline restore: a cashier who had a validated open shift before an
// outage must come back to the SAME branch + open shift after an offline restart,
// never "Branch unavailable / No open shift" - and the terminal must NEVER
// fabricate a shift, widen a branch, or restore another context's session. These
// tests exercise the durable POS-session snapshot directly (a pure localStorage
// module) and pin the wiring by reading source.
...
function makeSnapshot(over: Partial<PosSessionSnapshot> = {}): PosSessionSnapshot {
  return {
    ...
    branch_name: "Main",
    currency: "USD",
    shift: { id: SHIFT, status: "open", opened_at: "2026-09-22T09:00:00Z", opening_cash_amount: 100, branch_id: BRANCH },
    ...over,
  };
}
